...
import static de.labathome.GenerateTestData.pythonName;
		...
		try (FileWriter fw = new FileWriter("src/test/python/GeneratedApiTests.py")) {
			try (PrintWriter pw = new PrintWriter(fw)) {
				...
				pw.println("# -*- coding: utf-8 -*-\n" +
						...
						"s = "+de.labathome.GenerateTestData.scalingFactorStr+"\n" +
						"class GeneratedApiTests(unittest.TestCase):\n");

				for (int time_dtype_idx = 0; time_dtype_idx < time_dtypes.length; ++time_dtype_idx) {
					final byte time_dtype = time_dtypes[time_dtype_idx];
					final String tT = BinaryTimeseries.dtypeStr(time_dtype);
					String ptT = pythonName.get(time_dtype);

					for (int scaling_dtype_idx = 0; scaling_dtype_idx < scaling_dtypes.length; ++scaling_dtype_idx) {
						final byte scaling_dtype = scaling_dtypes[scaling_dtype_idx];
						final String tS = BinaryTimeseries.dtypeStr(scaling_dtype);
						String ptS = pythonName.get(scaling_dtype);

						for (int data_dtype_idx = 0; data_dtype_idx < data_dtypes.length; ++data_dtype_idx) {
							final byte data_dtype = data_dtypes[data_dtype_idx];
							final String tD = BinaryTimeseries.dtypeStr(data_dtype);
							String ptD = pythonName.get(data_dtype);
							...
							pw.println(		"        for i in range(10):\n" + 
									"            raw_data[i] = "+ptD+"(o+i*s)");
							if (scaling_dtype != BinaryTimeseries.DTYPE_NONE) {
								if (data_dtype == BinaryTimeseries.DTYPE_FLOAT || data_dtype == BinaryTimeseries.DTYPE_DOUBLE
										|| scaling_dtype == BinaryTimeseries.DTYPE_FLOAT || scaling_dtype == BinaryTimeseries.DTYPE_DOUBLE) {
									// reference values are floating point
									pw.println("        scaled_data = np.add(np.multiply(raw_data, "+ptS+"(s), dtype=np.float64), "+ptS+"(o))");
								} else {
									// reference values are integers
									pw.println("        scaled_data = np.add(np.multiply(raw_data, "+ptS+"(s), dtype=np.int64), "+ptS+"(o))");
								}
							}
